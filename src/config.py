...
# 2000未満に収めること。
ALWAYS_POOL_CAPACITY: int = int(env_get("CALM_ALWAYS_POOL_CAPACITY", "1500"))

# habits DBから ~/.claude/rules 配下へ投影する自動生成ファイルの書き込み先パス
HABITS_RULES_PATH: str = env_get("CALM_HABITS_RULES_PATH") or str(
    Path.home() / ".claude" / "rules" / "cc-memory-habits.md"
)
# 投影のkill switch。"0"で無効化すると、以後の投影はプレースホルダ本文で
# 上書きされたまま停止する（stale化したファイルが注入され続けるのを防ぐ）
HABITS_RULES_EXPORT_ENABLED: bool = env_get("CALM_HABITS_RULES_EXPORT", "1") != "0"
...
CALM_MIGRATION_SNAPSHOT: bool = env_get("CALM_MIGRATION_SNAPSHOT", "1") != "0"
# 実DBコピーへのdry-run適用ゲートを無効化する緊急脱出弁（"0" で無効化）
CALM_MIGRATION_DRYRUN: bool = env_get("CALM_MIGRATION_DRYRUN", "1") != "0"
...
TOTAL_INJECTION_BUDGET_CHARS: int = int(env_get("CALM_TOTAL_INJECTION_BUDGET_CHARS", "10200"))

# --- Precedent pull ---
# 本文展開（decision + reason）の予算（文字数）。index行・material snippet・routing
# メタデータは対象外（別途有界のため予算計算に含めない）
PRECEDENT_BUDGET_CHARS: int = int(env_get("CALM_PRECEDENT_BUDGET_CHARS", "24000"))
PRECEDENT_ROUTING_K_MAX: int = 5
# topic_vec KNNで取得する候補数（selected上限のPRECEDENT_ROUTING_K_MAXより広めに取る）
PRECEDENT_ROUTING_CANDIDATES: int = int(env_get("CALM_PRECEDENT_ROUTING_CANDIDATES", "10"))
...
PRECEDENT_ROUTING_MISS_DISTANCE: float = float(env_get("CALM_PRECEDENT_ROUTING_MISS_DISTANCE", "0.19"))
