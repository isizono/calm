...
以下の情報を元に、ユーザーの質問に合わせてCALMの使い方を説明してください。
...
CALMは「着手 → 記録 → 完了 → 同期」のライフサイクルで文脈を維持する。
...
- 一度作れば、以降は基本サイクルで運用していく
- **タイミング**: 新しいプロジェクト・取り組みでCALMを使い始める最初の一度
...
- **`/scribe`** — CALMに蓄積された記録（決定事項・ログ・資材など）からドキュメントを生成する。ADR、議事録、設計ドキュメントなどを書き起こしたいときに
...
- **起票**（activity-start）: 終了条件を書く／不要印を付ける／何も書かない、の3択にする。雛形で埋めない。条件は真偽の付く1文で書き、Claudeが推した条件はチャットで示して追認を得てから書く
- **記録の直後**（recording・decision-record）: 条件を満たしたら、その場でupdate_goalに書く。根拠のdecisionなどがあれば、同じ呼び出しで束縛も張る。それで判定待ちになったら、そのまま判定に続く
- **待ち**（ask-compose・activity-pause）: 人間の判断待ちは、その場で聞けるなら聞いてdecisionにする。離席中・セッション跨ぎならaskにして、条件をそのaskに束縛する
- **判定**（recording・decision-record・check-in・activity-finish・activity-cleanup）: 全条件が終端になった判定待ちの状態を見たら、そのセッションがその場でjudge_goalを呼ぶ。未決（openなask・[議論中]のdecision）があれば、判定の前に畳むか1ターン聞く
- **事後の記録**（sync-memory）: そのセッション自身に根拠がある充足だけを書く。推定でしかない外部の完了は条件にも判定にも使わず、judge_goalも呼ばない

### 終了条件を書かない選択

不要印（`set_goal`の不要印）と未定義（何も書かない）は、どちらも正当な状態。常駐の活動や同じセッションで閉じる小作業には不要印、まだ終わりが見えない作業は未定義のまま進めてよい。書くことは強制されず、check_inの次の一手が促すだけにとどまる。
...
Ask storeは、だいたいこういうデータフローを提供する仕組み。

作業中のAIがあなたに聞きたいことがあるけど、すぐに答える必要はない——そういうときにAIはその問いをAsk storeに登録する。次にどのセッションを開いても、起動した瞬間にその問いが自然と目に入る。答えると、元の作業をしていたセッションにその判断が伝わり、AIはそれを踏まえて作業を続ける。

つまり「今すぐ答えなくていい判断」を、あなたの好きなタイミングで拾えるようにする仕組み。

Ask storeの読み書き（`add_ask`・`answer_ask`・`triage_ask`・`withdraw_ask`）と完了通知ファイルの書き込みは、いずれもrelayと無関係に動作する。relay未接続時はrelay経由のクロスセッション通知が静かに省略されるだけで、ask自体の作成・回答・トリアージの成否には影響しない。

### 基本の使い方

- 判断待ちが溜まっていないか気になったら「asks見せて」とAIに聞くか、`/ask-answer`で一覧して1件ずつ答えられる
- 回答するときは、そのまま会話であなたの判断を伝えればAIが記録してくれる
- 回答しただけではその問いが元のactivityをブロックしている状態は解除されない。解除されるのは`triage_ask`（promote/dismiss）または`withdraw_ask`が実行されたときのみで、AIの自走裁定や次回のcheck-in等で拾われるまで待つ形になる

### どのくらいの粒度で聞かれたいかはあなたが決める

「これくらいなら聞かなくていい」「これは必ず聞いてほしい」の線引きは人によって違う。自分なりの運用ルールをAIに伝えておくと、Askの粒度がその通りになっていく。
...
CALMは、セッション間で文脈を保持するためのツール。
セッションが終われば会話は消えるが、CALMに記録された情報は次のセッションに引き継がれる。
...
CALMは使い続けるほど価値が出るツールだが、放っておいて勝手に良くなるわけではない。
...
- **プロジェクトとディレクトリの対応**: 「このリポジトリは〇〇プロジェクトのバックエンド」「△△はこのmonorepo内のpackages/appにある」など。`domain:`タグと紐づけておくと、check-in時の文脈取得が的確になる。ここに書ける情報は、リポジトリ内の相対パス・remoteのURLや`owner/repo`形式の識別子・サービス名など、書いた本人以外のマシン・セッションからも解決できる参照に限る（上の2例はどちらもこれに該当する）。tag-notesはcheck-in時に他セッションへ自動注入され、export機能で別インスタンスにも渡りうるため、自分のマシンでしか通じないローカル絶対パス（`~/`・`/Users/...`等）は書かない。そうした情報を残したい場合は`/remember`の判定に委ねる
- **ユビキタス言語**: プロジェクト固有の用語や略語。AIが勝手に別の言い回しで記録すると、後から検索で引っかからなくなる

#### データ保護（スナップショット）

CALMはセッション開始時（SessionStart hook）にDBの自動スナップショットを取得する。デフォルトでは12時間間隔で、最新5世代を保持する。スナップショットはDBと同階層の `snapshots/` ディレクトリに保存される。
