---
name: db-recovery
description: 【必須】SessionStart hookがDBテーブル行数の大幅減少警告(🚨🚨🚨 DBデータ異常減少を検知)を注入した状況、またはセッション中に自分でDB件数の異常な少なさ・直近記録の消失に気づいた状況で発動する。スナップショット所在確認 → 復旧方法(A: 全体復元 / B: 部分復元 / C: 復旧せず先に進む)の提示・承認 → 承認後の復元実行 → 再検証、まで一貫して進める自律発動スキル。このスキルを経由せずに異常警告を放置したり、calm:manの手動手順だけを案内して終わらせたりしてはいけない。TRIGGER: SessionStart時にDB異常警告を受領した時、会話中にactivity/decision/log等の想定件数の少なさに気づいた時、ユーザーから「データ消えてない?」「件数減ってない?」と指摘された時 など。DO NOT TRIGGER: 警告が無く通常のDB状態の場合、直前の正当な操作(大量retract・tag-cleanup等)による説明可能な減少と確認済みの場合、ユーザーが既に別経路で復旧完了したと明言した場合。
---
...
6. 実行結果の提示後、サーバー再起動について改めてユーザーの明示承認を取る。上記1.の一括承認は再起動を含まないため、ここで新たにyes/noを得るまで再起動を実行しない。承認が得られたら`/restart`スキル(calm:restart)に委譲して再起動する
