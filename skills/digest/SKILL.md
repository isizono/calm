...
指定期間（デフォルト直近7日間）に動きのあったアクティビティ・決定事項・成果物をdomainごとに俯瞰するダイジェストを生成する。ユーザーに期間内の動きを伝えることがゴール。読み取り専用skillであり、記録・更新は一切行わない。

## 手順

### 1. 期間の決定

引数や会話中の期間表現があれば最優先で使う。
...
期間表現が無ければ、デフォルトの上限を直近7日間（today - 7日 〜 today）とした上で、文脈量に応じて短縮するか判定する。

1. `get_activities(status="active", since=today-7日, limit=1)`と`get_activities(status="completed", since=today-7日, limit=1)`の`total_count`を合算し、直近7日間で動いたアクティビティ数を確認する
2. 合算値が15件を超える場合は直近3日間に短縮する。超えない場合は7日間のまま使う（＝「7日」と「文脈量に応じた短縮後の期間」のうち短い方を採用する）
3. 実際に採用した期間（7日間 or 3日間）とその理由（短縮した場合は「直近7日間はN件と多いため直近3日間に絞った」）を出力冒頭に明記する
