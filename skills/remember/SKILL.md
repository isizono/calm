---
name: remember
description: 【必須】ユーザーが「覚えて」「保存して」「これ残して」「忘れないで」「メモって」「記録して」など、何かを記憶・永続化することを依頼したときに発動。情報の保存先を判定するフローを提供する。このスキルを経由せずにauto-memory・CLAUDE.md・habits・tag-notes・rules・pinに直接書いてはいけない。TRIGGER: ユーザーが何かを覚えておくこと・保存すること・記憶に残すことを依頼した時、表現のバリエーションは問わない。または、ユーザーの発言に既視感があり(前にも同じ内容を言われた気がする)場合、CALM searchで裏取りしたうえで発動する(雑談レベルの繰り返しは対象外)。DO NOT TRIGGER: 議論の合意事実の記録（decision-record skillの担当）、経緯・成果物の記録（recording skillの担当）、過去の記録の訂正・撤回（forget skillの担当）、メタask裁定の発効に伴う配置（rule-placement skillの担当）。
---
...
ユーザーが明示的に記憶を依頼していなくても、発言に既視感を感じたら(雑談レベルの繰り返しは対象外)、CALM searchで裏取りする。

- searchでヒットしない → 勘違いとして扱い、以降の判定フローには進まず通常の応答に戻る
- ヒットして、まだどこにも記録されていない → 下記の判定フローへ進み、新規保存として扱う
- ヒットして、既にauto-memory/habits/rules/tag-notes等に記録済み → 本スキルの対象外。なぜ効かなかったか（配置が弱い/内容が曖昧等）を一次切り分けしたうえで、[rule-placement](../rule-placement/SKILL.md) skillのfull評価へ委譲する
...
- YES → 全文の正は**判例decision**（add_decisionsで記録。reasonに適用条件:/適用外:の定型節を含める。書式は本スキル同梱の `references/precedent-format.md`）と決まる。続けて下の判定フローで契機を判定し、契機の置き場が決まった場合はそこへ**要旨と判例への参照**を書く（両方配置。実行時は判例decision作成→参照配置の順）
- NO → そのまま判定フローへ
...
│
├─ 特定のタグ文脈でだけ必要？
│   └─ YES → tag-notes（update_tagで記録。条件付き判断基準なら、
│       tag-notesには要旨+判例への参照を書く）
│
├─ どのプロジェクト・文脈でも常に適用される？
│   ├─ 条件付き判断基準（前判定YES）？
│   │   └─ 判例decisionのみで完結（追加の置き場は作らない。契機を名指し
│   │       できない条件付きは実質「自覚的な迷い」が契機であり、迷った瞬間の
│   │       search / add_ask同梱のpullで届く判例が正しい置き場）
