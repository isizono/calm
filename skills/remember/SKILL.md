...
- YES → 全文の正は**判例decision**（add_decisionsで記録。reasonに適用条件:/適用外:の定型節を含める。書式は docs/precedent-format.md）と決まる。続けて下の判定フローで契機を判定し、契機の置き場が決まった場合はそこへ**要旨と判例への参照**を書く（両方配置。実行時は判例decision作成→参照配置の順）
- NO → そのまま判定フローへ
