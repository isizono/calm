...
4. 既存の notes がある場合はその内容を保持しつつ、新しい内容をマージしたドラフトを作成する
   ...
   `#direction-overflow-ack`, `#activity-cleanup-skipped`, `#notes-over-budget-ack`）を
   追記・更新する場合の書式は以下の通り:
   ...
   - 例外: `#notes-over-budget-ack`はマーカー単体（恒久抑制）を受け付けない。notesが
     長すぎる状態を恒久的に黙らせるのは望ましくないため、期限付き抑制（-until:）のみ
     有効とする。ただしこのマーカーが発火する対象タグは既にnotesが上限を超えており、
