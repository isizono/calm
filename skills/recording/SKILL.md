...
sync-memory による漏れの救済は、ユーザーが `/sync-memory` を明示的に実行した場合に限られる（自動実行はない）。そのためrecording skillは「完全網羅」ではなく「セッション中の見逃し低減」が役割。判断に迷ったら記録する側に倒す。
...
迷ったら記録する。漏れの最終救済はユーザーが `/sync-memory` を実行した場合の sync-memory が担うが、本スキルの役割は「セッション中の見逃し低減」であり、ここで取りこぼすと sync-memory の transcript 解析時には文脈が薄れている可能性がある。
