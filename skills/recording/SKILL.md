...
セッション終了時には sync-memory が漏れを救うため、recording skill は「完全網羅」ではなく「セッション中の見逃し低減」が役割。判断に迷ったら記録する側に倒す。
...
迷ったら記録する。漏れの最終救済は sync-memory が担うが、本スキルの役割は「セッション中の見逃し低減」であり、ここで取りこぼすと sync-memory の transcript 解析時には文脈が薄れている可能性がある。
