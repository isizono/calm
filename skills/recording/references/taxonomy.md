...
以下は各呼び出し元が実際に使う記録先カテゴリの運用上の対応表であり、0章で述べた「本ドキュメントへの直接参照の有無」とは別軸である。sync-memory skill は本ドキュメントを直接参照しないが、記録先カテゴリとしては log/material/decision/report_signal のすべてを扱う（decision は sync-memory 側の Step 2 に判定基準がインラインで存在する）。
