---
name: sync-memory
description: セッション終了前にtranscriptを解析し、トピック・決定事項・ログ・アクティビティを一括で記録・更新する。「/sync-memory」「同期して」「今日の分を記録して」「セッション終わるから残しておいて」など、セッション内容の一括記録の意図で発動する。bgセッションを止める前、複数のorchの窓口を同時に閉じるときなど人が読まない場面では `--minimal` 引数で最小形（未記録区間の記録とアクティビティの始末のみ）を実行する。個別の経緯・成果物のその場記録はrecordingが担当する。
---
...
引数なしなら標準形（下記1〜6）を実行する。`--minimal` 引数（`/sync-memory --minimal` など）が渡された場合は[最小形](#最小形-minimal)を実行する。

このセッションが`orch`タグの付いたアクティビティ（orch）の担い手で、ユーザーがこの窓口を閉じると言っているなら、以下の手順に入る前に[orch](../orch/SKILL.md) skillの「空ける」を先に実行する。閉じないsync-memory（サーバー再起動の前、途中の記録など）では空けない。
...
ステップ1で特定した範囲を、トピック・資材・決定事項・ログ・関連に振り分けて記録する。

#### トピックの特定・作成 (add_topic)
...
- 保存した material ID は決定事項・ログの記録で参照する

#### 決定事項・ログの記録 (add_decision / add_log)
...
- log には保存した material ID を参照として含める（例: 「分析結果（material #NNN）を踏まえてA案を採用」）
- `topic_id` は関連するトピックを指定
- `tags` で内容を表すタグを付ける（省略時はtopicのタグを継承）
- **記録したdecisionの中に、今後の作業や継続議論を伴うものがあれば、対応するアクティビティを `add_activity` で作成する**（ステップ3で未登録の場合）

CALM自身の不具合・使用感の違和感・既存記録との矛盾に気づいた場合は
...
#### 未決定の論点を記録 (add_decision)

議論が途中で終わった論点・アイデア・検討事項を記録する。**決定していなくても記録する**。

`[議論中]` の記録基準の正本は `decision-record` skill の「未決論点の記録: `[議論中]`」節であり、本ステップの記述と食い違う場合はそちらに従う。`[未完]` はアクティビティ側の関心事であり、ステップ3で扱う。
...
#### リレーションの確認・追加 (add_relation)

記録したトピック・アクティビティ間の関連を `add_relation` で追加する。**ここまでトピック・アクティビティの新規作成も既存エンティティへの記録もなかった場合はこの節をスキップする。**
...
### 3. アクティビティの始末（自セッションが触ったものに限る）

自セッションが触った（言及・着手・作業した）アクティビティに限り、次の3つを行う。全アクティビティを対象にした整理はステップ4a（棚卸し）の役割であり、ここでは扱わない。

#### 未登録の作業を起票する

実装予定・作業予定をすべて登録する。ここで登録したアクティビティは、資材保存の紐づけ先になる。
...
判定対象は、節見出しのとおり自セッションが触ったアクティビティに限る。以下の基準（フェーズ遷移済み等）はDB状態だけで機械的に真偽が付くが、それでも自セッションが触っていないアクティビティには適用しない。ステップ1で取得した全アクティブアクティビティのうち、自セッションが触っていないものは、基準に該当していてもここでは判定しない（該当すればステップ4aの棚卸しの対象になりうる）。
...
description: "「sync-memory改善」トピックで議論中。決定事項の記録はそのまま残し、追加でadd_activityでも登録する方針で合意済み（decision A案）。残作業：SKILL.mdへのステップ追加と動作確認。"
```

**注意:**
- 未決定の論点を記録する節の `[未完]` decision とは別に、アクティビティとしても登録する（役割が異なる：decisionは「何が未決定か」、アクティビティは「次に何をすべきか」）
- 未完の話がなければこの節はスキップする
- transcript中でこの作業の終了条件が明言されている、または推せるなら、起票時に上記「未登録の作業を起票する」節の手順で確認なしに `set_goal(new)` を書く。候補が複数で定まらない、またはそもそも推せないときは書かない

### 4. 棚卸し・remember（自動実行）

ステップ4a〜4bの判定に基づき、ユーザーに確認せずClaudeが判断してその場で処理する。処理内容はステップ6の完了報告にまとめて記載する。**4a〜4bが何も無ければステップ4全体をサイレントスキップする。**

#### 4a. アクティビティの棚卸し

（本節の判定基準が `recompose-context` skill の全体モードの「3. 処遇判定」節と食い違う場合は、そちらを正本とする）

**goal付きアクティビティの扱い:** 以下の判定でcompletedにする（または重複統合で閉じる側にする）前に、対象が goal に紐づいていないか `get_goal(activity_id=...)` で確認する。goal に紐づいている場合、この棚卸しの推定だけではcompletedにせず、judge_goal も呼ばない（このセッションに根拠のある充足だけが判定材料になり得るが、推定に基づくこのステップの判定はその根拠にならない）。ただし外部の完了を、ステップ3の確信度「高」と同じ基準（`gh` など実際の手段で確かめられれば `update_goal` で該当条件を `satisfied` に書き、終端すれば `judge_goal` も呼んでよい）で確認できた場合はその限りではない。重複判定（下記1）でも、goal付きのアクティビティは閉じる側にしない（残す側にする。両方がgoal付きなら、このステップでは統合せず現状のまま残す）。

**まずスキップ判定:** ステップ1で取得したアクティビティを以下のカテゴリに照合し、**該当が1件もなければ即座にスキップする。** ユーザーにアクティビティ一覧を見せたり、「該当なし」と報告する必要はない。

**判断基準 — いずれかに該当するアクティビティがある場合のみ処理対象とする:**

（確信度「高」のアクティビティはステップ3で自動クローズ済みのため、ここでは扱わない）

**確信度「中」（完了の可能性が高いがエージェント判断では確定できないもの）:**
- 前回セッションであと一歩だったもの（残タスクが軽微）
- 外部で完了した可能性が高いもの（transcript外の状況変化を示唆する情報がある）
→ `update_activity(status="completed", closed_by="external")` で自動的にcompletedにする（ステップ3の確信度「高」と同様の扱い）
...
2. **放置**: `max(updated_at, last_heartbeat_at)` から7日以上経過しているpendingアクティビティ → 前提が変わらずまだ必要と判断すれば `update_activity(activity_id, status="snoozed")` で寝かせる（snooze期間経過後に自動でpendingに復活する）、状況を見て不要と判断すれば `update_activity(status="completed", closed_by="claude")` にする
...
判断に迷うものは書き込まない（誤って書くより書かないほうが安全）。tag notesは`update_tag`が全文置換である点に注意し、既存notesを失わないよう必ずマージしてから書き込む。記録すべきものがなければ4bは空とする。
...
4. 候補ごとの `candidate.degraded`（内部で行われた `search` バッチ呼び出しの `degraded` を候補単位に反映したもの）で `degraded=true` のときは判定を保守側に倒す（該当候補についてはprecedent_miss記録を行わない。skipした事実は完了報告に一行残す）
...
### アクティビティ
- (登録・完了にしたアクティビティを列挙)
- (ステップ3で確信度「高」として自動completedにしたものを列挙。理由も記載)
- (ステップ4aの棚卸しで処理したアクティビティを列挙)
- ※ 誤りがあれば戻せます
...
- **30**: ジャックポット。`jackpot.md` を読み、対応表の項目数を数えて `roll_dice(sides=<項目数>)` を振ってネタを決定する。ダイスには触れず、選ばれたネタをそのまま実行する
...
bgセッションを止める前、複数のorchの窓口を同時に閉じるときなど、人がその場で読まない場面で使う。orchが複数のbgセッションを続けて止めるとき、または複数のorchの窓口を同時に閉じるときは、標準形の棚卸し（全アクティブアクティビティを対象にする）が並行して動く他セッションの作業に触れてしまうことと、rememberの全文置換書き込みが競合することを避けるため、最小形にする。1つの窓口だけを閉じるときは、この競合が起きないため標準形のままにする。

- ステップ1〜3を標準形と同じ基準で実行する（ステップ3の「自セッションが触ったものに限る」がそのまま効く）
- ステップ4（棚卸し・remember）・ステップ5（聞き返し検出）は実行しない
- 完了報告は次の簡略形にする。ふりかえりは標準形と同じ手順を実行する（省略しない）

```
## sync-memory 完了（最小形）

- 記録: トピックN件・資材N件・決定事項N件・ログN件
- アクティビティ: 登録N件・自動クローズN件・[未完]引き継ぎN件
...
- **既に記録済みのものは重複して記録しない** - ステップ1で取得した既存データと照合すること
- **現在進行中の作業がある場合は `add_activity` で登録し、`update_activity` で `in_progress` にする**
- **ステップ1〜5（棚卸し・rememberを含む）ではツール呼び出しの前後に判断理由・中間分析・進捗報告のテキストを出力しない。見せるのはステップ6の完了報告のみ（最小形も同様、簡略形の完了報告のみ）**
