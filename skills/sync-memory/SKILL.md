...
- 確信度「高」のアクティビティは `update_activity` で `completed` にする（自動クローズしたものはStep 9で事後報告する）
- 自動クローズしたアクティビティに次フェーズがあれば、アクティビティを作成する（例: `[議論]` 完了 → `[設計]` を作成）。既に存在すればスキップ。確信が持てなければStep 9cでユーザーに確認する
