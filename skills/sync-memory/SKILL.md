...
- 起票するアクティビティについて、transcript中でこの作業の終了条件が明言されていれば、起票と同じタイミングで `set_goal(activity_id, goal={"new": {...}})` をopenの条件として書く（Claudeが推測した条件は書かない。明言されていなければ何も書かず未定義のままにする）
- 確信度「高」のアクティビティは `update_activity` で `completed` にする（自動クローズしたものはStep 10で事後報告する）。ただし対象が goal に紐づいている場合は `get_goal(activity_id=...)` で確認し、completed にしない（このセッションの sync-memory はgoal付きアクティビティを完了にせず、judge_goal も呼ばない。推定の外部完了は goal の判定材料にしない）
...
- transcript中でこの作業の終了条件が明言されていれば、起票時にStep 2の手順で `set_goal(new)` を書く（Claudeが推測した条件は書かない）
