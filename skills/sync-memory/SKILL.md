---
name: sync-memory
description: セッション終了前にtranscriptを解析し、トピック・決定事項・ログ・アクティビティを一括で記録・更新する。「/sync-memory」「同期して」「今日の分を記録して」「セッション終わるから残しておいて」など、セッション内容の一括記録の意図で発動する。bgセッションを止める前など人が読まない場面では `--minimal` 引数で最小形（未記録区間の記録とアクティビティの始末のみ）を実行する。個別の経緯・成果物のその場記録はrecordingが担当する。
---
...
（本節の判定基準が recompose-context の全体モード（`skills/recompose-context/references/full-inventory.md`）の棚卸し基準と食い違う場合は、そちらを正本とする）
...
bgセッションを止める前など、人がその場で読まない場面で使う。orchが複数のbgセッションを続けて止めるとき、標準形の棚卸し（全アクティブアクティビティを対象にする）が並行して動く他セッションの作業に触れてしまうことと、rememberの全文置換書き込みが競合することを避けるため。
