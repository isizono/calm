"""migration 0055_add_migration_ledger のテスト
...
@pytest.fixture
def db_before_0055():
    """0048 までの migration を適用した DB を提供する。0055 の挙動を分離検証するために使う。"""
    with tempfile.TemporaryDirectory() as tmpdir:
        db_path = os.path.join(tmpdir, "test.db")
        os.environ["DISCUSSION_DB_PATH"] = db_path
        ...
        pre_0055 = MigrationList([m for m in all_migs if m.id < "0055"])
        with backend.lock():
            backend.apply_migrations(pre_0055)
...
def _apply_migration_0055(db_path: str) -> None:
    """db_path に対して migration 0055 のみを適用する。"""
    parsed = parse_uri(f"sqlite:///{db_path}")
    backend = _VecSQLiteBackend(parsed, default_migration_table)
    all_migs = read_migrations(str(MIGRATIONS_DIR))
    only_0055 = MigrationList([m for m in all_migs if m.id.startswith("0055")])
    with backend.lock():
        backend.apply_migrations(only_0055)


class TestMigrationLedgerTableCreated:
    def test_table_exists_after_0055(self, migrated_db):
        conn = get_connection()
        try:
            assert table_exists(conn, "migration_ledger"), (
                "migration_ledger テーブルが 0055 適用後に存在しない"
            )
        finally:
            conn.close()

    def test_table_not_exists_before_0055(self, db_before_0055):
        conn = get_connection()
        try:
            assert not table_exists(conn, "migration_ledger"), (
                "0055 適用前に migration_ledger テーブルが既に存在している"
            )
        finally:
            conn.close()
    ...
    def test_required_columns_exist(self, migrated_db):
        ...
        try:
            ...
            for col in {"migration_id", "content_sha256", "applied_at"}:
                assert col in column_names, f"migration_ledger.{col} が 0055 適用後に存在しない"
        finally:
            conn.close()
...
class TestExistingRowsUnaffected:
    def test_0055_does_not_touch_other_tables(self, db_before_0055):
        """0055 は追加専用のmigrationであり、既存テーブルの行を変更しない"""
        conn = get_connection()
        try:
            conn.execute(
        ...
        _apply_migration_0055(db_before_0055)
