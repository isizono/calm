...
def test_check_capability_matches_matrix(
    tool_name, role, expected, migrated_db, monkeypatch
):
...
def test_matrix_does_not_have_extra_tools():
    ...
    async def _list_for_extras():
        return await mcp.list_tools()

    tools = asyncio.run(_list_for_extras())
    registered_names = {t.name for t in tools}
    matrix_names = set(CAPABILITY_MATRIX.keys())
...
class TestEscalationAcrossRoles:
    ...
    def test_escalation_unblocks_denied_tool(self, role, migrated_db, monkeypatch):
        monkeypatch.setenv("OW_ROLE", role)
        ...
        check_capability(denied)


class TestUserRoleAbsence:
    """matrix に未登録の role (例: "user") はデフォルト deny。"""

    def test_unknown_role_denies_all(self, migrated_db, monkeypatch):
