...
Agent呼び出し時の `model` パラメータは省略する（`task-reviewer` 定義ファイル側でopus 4.7のフルIDに固定済み。バグ検出・仕様整合・テスト審査の判断品質を優先するため、親がsonnet運用でもレビューはopus 4.7で動く）。
...
3. テスト仕様カバレッジ（照合）: plan.md「## エッジケース（仕様カバレッジ）」表の
