...
| 項目 | 予算（字） | 保証種別 | 備考 |
|---|---:|---|---|
| RULES全文 | ≤2,048（安全マージン1,900、現状は超過中） | complete | MCPクライアント側で2,048字を超えると切り詰められる実態があるため、全文がこのハードリミット以内に収まることを回帰テストで保証する。安全マージンとして1,900字以内を目指すが、確定済み運用ポリシー文言の追加により現状は超過している（既知の超過としてxfailで追跡）。先頭にコンテキスト取得原則・アクティビティ運用・記録の使い分け・タグ運用を置き、末尾に `cc-memory:guide` skillへの導線1行を置く |
