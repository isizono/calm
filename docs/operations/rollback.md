...
本ドキュメントは PR を取り消す（revert する）ときの手順を、R1/R2 の 2 分類ごとに 1 ページでまとめる。PR テンプレートに本文書へのポインタとなる「Revert」セクションを設ける案があるが、本書執筆時点で `.github/PULL_REQUEST_TEMPLATE.md` には未反映（別途対応予定）。
