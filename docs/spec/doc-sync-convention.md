...
サーバー本体（calm MCP server）はリポジトリ側ドキュメントを読まない。calm サーバーは任意ユーザーの汎用サーバーであり、特定リポジトリのファイル配置を知る立場にないためである。本規約が定める検査は本リポジトリ専用の `scripts/` 配下のローカルスクリプトと CI の 1 ステップで完結し、サーバー本体には一切手を入れない。
