...
**返り値**: `{goal_id_raw, handle, statement, label, progress, claude, next, last_verdict, conditions: [...全件...], activities: [...], logs_since_created: {count, since}, open_questions?, open_questions_more?, children?, attention?, attention_more?}` | `{label: "undefined"|"not_needed", next?, reason?}`（activity_idを指定してgoalが無い場合）。`children`/`attention`/`attention_more`はcheck_inのgoalブロックと同じ形（2.18参照）で、activity束縛の条件が1件以上あるときだけ付く。
