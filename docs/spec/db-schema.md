...
| `obs_kinds` / `lesson_kinds` / `delivery_channels` | — | 観測台帳・知見・配達経路の許容値を持つ参照表 |
...
- `UNIQUE(session_id, src_uuid)` と部分UNIQUEインデックス `uq_obs_speaker`（`ref_id` に `kind='speaker'` の行を1件に絞る）により、同じ発話・返信を二重記録しない
